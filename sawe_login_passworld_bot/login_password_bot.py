...
@dp.message_handler(Text(equals=['добавить аккаунт', 'получить информацию', 'удалить один аккаунт'], ignore_case=True))
...
async def send_info(message: types.Message, state: FSMContext):
    if message.text == 'получить информацию':
        ...
        await bot.send_message(message.from_user.id, 'введите логин аккаунтаб который хотите удалить',
